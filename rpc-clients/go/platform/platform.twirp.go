...
/*
Package platform is a generated twirp stub package.
...
// =======================
// AuthSvc Protobuf Client
// =======================

type authSvcProtobufClient struct {
	client HTTPClient
	urls   [1]string
}

// NewAuthSvcProtobufClient creates a Protobuf client that implements the AuthSvc interface.
// It communicates using Protobuf and can be configured with a custom HTTPClient.
func NewAuthSvcProtobufClient(addr string, client HTTPClient) AuthSvc {
	prefix := urlBase(addr) + AuthSvcPathPrefix
	urls := [1]string{
		prefix + "Token",
	}
	if httpClient, ok := client.(*http.Client); ok {
		return &authSvcProtobufClient{
...
func (c *authSvcProtobufClient) Token(ctx context.Context, in *TokenReq) (*TokenRes, error) {
	ctx = ctxsetters.WithPackageName(ctx, "com.abpsamtwirp.platform")
	ctx = ctxsetters.WithServiceName(ctx, "AuthSvc")
...
type authSvcJSONClient struct {
	client HTTPClient
	urls   [1]string
}

// NewAuthSvcJSONClient creates a JSON client that implements the AuthSvc interface.
...
func (c *authSvcJSONClient) Token(ctx context.Context, in *TokenReq) (*TokenRes, error) {
	ctx = ctxsetters.WithPackageName(ctx, "com.abpsamtwirp.platform")
	ctx = ctxsetters.WithServiceName(ctx, "AuthSvc")
	ctx = ctxsetters.WithMethodName(ctx, "Token")
	out := new(TokenRes)
...
func (s *authSvcServer) writeError(ctx context.Context, resp http.ResponseWriter, err error) {
	writeError(ctx, resp, err, s.hooks)
}

// AuthSvcPathPrefix is used for all URL paths on a twirp AuthSvc server.
// Requests are always: POST AuthSvcPathPrefix/method
// It can be used in an HTTP mux to route twirp requests along with non-twirp requests on other routes.
const AuthSvcPathPrefix = "/twirp/com.abpsamtwirp.platform.AuthSvc/"

func (s *authSvcServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	ctx := req.Context()
	ctx = ctxsetters.WithPackageName(ctx, "com.abpsamtwirp.platform")
	ctx = ctxsetters.WithServiceName(ctx, "AuthSvc")
	...
	switch req.URL.Path {
	case "/twirp/com.abpsamtwirp.platform.AuthSvc/Token":
...
func (s *authSvcServer) serveToken(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
...
func (s *authSvcServer) serveTokenJSON(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "Token")
	...
	// Call service method
	var respContent *TokenRes
	func() {
		defer ensurePanicResponses(ctx, resp, s.hooks)
		respContent, err = s.AuthSvc.Token(ctx, reqContent)
	}()
	...
	if respContent == nil {
		s.writeError(ctx, resp, twirp.InternalError("received a nil *TokenRes and nil error while calling Token. nil responses are not supported"))
		return
	}
...
func (s *authSvcServer) serveTokenProtobuf(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "Token")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
...
func (s *authSvcServer) ServiceDescriptor() ([]byte, int) {
	return twirpFileDescriptor0, 0
}

func (s *authSvcServer) ProtocGenTwirpVersion() string {
	return "v5.7.0"
}

func (s *authSvcServer) PathPrefix() string {
	return AuthSvcPathPrefix
}
